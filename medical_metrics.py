...
class UltimateSegmentedGaitAnalyzer:
    ...
    1. 正面视角物理脱钩: 消除 2D 正面透视压扁对 Knee ROM 的伪误扣分；
    2. 透视去趋势 (Detrending): 消除受试者走近/走远时的透视基线漂移；
    3. 单目 2D 标尺校准: 将侧面 Knee ROM 物理标尺校准至单目 2D 视觉规范；
    ...
    def __init__(self, raw_video_fps: float = 30.0, min_segment_frames: int = 15):
        ...
        # 视角判定阈值
        self.shoulder_ratio_side_thresh = 0.15
        self.shoulder_ratio_frontal_thresh = 0.22
    ...
    def _detect_viewpoint(self, sub_df: pd.DataFrame, inst_torso_h: np.ndarray) -> Tuple[str, float]:
        ...
        if shoulder_ratio < self.shoulder_ratio_side_thresh:
            return "SIDE", round(shoulder_ratio, 3)
        elif shoulder_ratio > self.shoulder_ratio_frontal_thresh:
    ...
    def _analyze_sub_segment(self, sub_df: pd.DataFrame) -> Optional[Dict]:
        ...
        step_cv = float(np.std(step_lengths) / (step_mean + 1e-5)) if step_mean > 0 else 0.0
        stride_time_cv = float(np.std(step_intervals_sec) / (np.mean(step_intervals_sec) + 1e-5)) if len(step_intervals_sec) > 1 else 0.0

        even_steps, odd_steps = step_intervals_sec[0::2], step_intervals_sec[1::2]
        ...
        # 5. 视角自适应躯干摇晃与打分
        trunk_angle = np.degrees(np.arctan2(sh_x - hip_x, -(sh_y - hip_y)))
        ...
        # 6. 双支撑相占比
        la_v = np.sqrt(np.diff((la_x - hip_x)/smooth_scale, prepend=0)**2 + np.diff((la_y - hip_y)/smooth_scale, prepend=0)**2) * effective_fps
        ra_v = np.sqrt(np.diff((ra_x - hip_x)/smooth_scale, prepend=0)**2 + np.diff((ra_y - hip_y)/smooth_scale, prepend=0)**2) * effective_fps
        
        is_double_support = (la_v <= 0.35) & (ra_v <= 0.35)
        double_support_ratio = float(np.clip(np.mean(is_double_support), 0.15, 0.70))

        # 7. 节律打分与综合 GSI
        combined_cv = max(step_cv, stride_time_cv, gai * 1.2)
        score_rhythm = 100.0 - (combined_cv / 0.20) * 15.0 if combined_cv <= 0.20 else \
                       85.0 - (combined_cv - 0.20) / (0.38 - 0.20) * 35.0 if combined_cv <= 0.38 else \
                       max(0.0, 50.0 - (combined_cv - 0.38) * 120.0)

        score_support = 100.0 - (double_support_ratio / 0.45) * 15.0 if double_support_ratio <= 0.45 else \
                        85.0 - (double_support_ratio - 0.45) / (0.60 - 0.45) * 35.0 if double_support_ratio <= 0.60 else \
                        max(0.0, 50.0 - (double_support_ratio - 0.60) * 150.0)
    ...
    def process_single_csv(self, csv_path: str) -> Optional[Dict]:
        ...
        if 'detected' not in df.columns:
            logging.warning(f"缺少 'detected' 列: {csv_path}")
            return None
